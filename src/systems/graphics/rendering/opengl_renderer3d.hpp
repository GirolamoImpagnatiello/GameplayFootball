...
namespace blunted {
  ...
  class OpenGLRenderer3D : public Renderer3D {
    ...
    public:
      ...
      virtual bool SaveBackBuffer(const std::vector<std::string> &filenames);
      virtual bool SaveDepthBuffer(const std::string &filename, int width, int height, float cameraNear, float cameraFar);
      virtual bool SaveDepthBuffer(const std::vector<std::string> &filenames, int width, int height, float cameraNear, float cameraFar);
      virtual bool SaveColorBuffer(const std::string &filename, e_TargetAttachment attachment, int width, int height);
      virtual bool SaveColorBuffer(const std::vector<std::string> &filenames, e_TargetAttachment attachment, int width, int height);
